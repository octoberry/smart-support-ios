...
@interface ACEngine () <ACConnectionDelegate>
...
@property (nonatomic, strong) NSMutableDictionary  *currentChatRooms;
...
- (ACUserModel *)userModel {
    return self.userInfoModel.user;
}

#pragma mark - initialization

- (instancetype)initWithURL:(NSURL *)url alias:(NSString *)alias andApplicationId:(NSString *)appId {
    if (self = [super init]) {
        _URL = url;
        _applicationId = appId;
        
        _userInfoModel = [[ACUserInfoModel alloc] initWithAlias:alias andName:nil];
        self.apiProvider = [[ACAPIProvider alloc] initWithURL:self.URL alias:alias appId:appId andName:nil];
        ...
        [self md_registerMethod:@"token" condition:^BOOL{ return self_weak.authorizationToken != nil; } selector:@selector(tokenWithCompletion:)];
                    ...
                    andName:(NSString *)name
{
    if (self = [super init]) {
        ...
        _userInfoModel = [[ACUserInfoModel alloc] initWithAlias:alias andName:name];
        self.apiProvider = [[ACAPIProvider alloc] initWithURL:self.URL alias:alias appId:appId andName:name];
...
- (void)tokenExpireNotification:(NSNotification *)note {
    self.apiProvider.externalToken = nil;
...
- (void)innerConnect {
    if (!self.apiProvider) {
        self.apiProvider = [[ACAPIProvider alloc] initWithURL:self.URL alias:self.userInfoModel.user.alias appId:self.applicationId andName:self.userInfoModel.user.name];
    }
    ...
    if (!self.userInfoModel.user.userID) {
        ...
        //} else if (!self.userInfoModel.rooms) {
    } else if (!self.currentChatRooms) {
        // rooms.count could be zero, but rooms should be not nil (!)
        // check rooms...
...
- (void)tokenWithCompletion:(void(^)(NSError *error))completion {
    ...
    [self md_dependencyWithName:@"connect" error:^(NSError *error) {
        ...
        if (!self.authorizationToken) {
            if (!self.apiProvider.externalToken)
            {
                if ([self.delegate respondsToSelector:@selector(getSystemAuthorizationKey)]) {
                    self.apiProvider.externalToken = [self.integrationDelegate getSystemAuthorizationKey];
                }
                else
                    completion(nil);
            }
...
-(void)updateToken:(void (^)(NSError *error, NSString *token))completion
{
    ...
    [self md_dependencyWithName:@"connect" error:^(NSError *error) {
        ...
        typeof(self_weak) self = self_weak;
        [self.apiProvider apiUpdateTokenWithCompletion:^(NSError *error, NSString *token) {
            if (error==nil) {
                if (token) {
                    self.apiProvider.authorizationToken = token;
...
- (void)connectAuthorize {
    ...
    if (!self.authorizationToken) {
        if (!self.apiProvider.externalToken)
        {
            if ([self.integrationDelegate respondsToSelector:@selector(getSystemAuthorizationKey)]) {
                self.apiProvider.externalToken = [self.integrationDelegate getSystemAuthorizationKey];
            }
        }
        [self.apiProvider apiTokenWithCompletion:^(NSError *error, NSString *token) {
...
- (void)connectRooms {
    ...
    [self.apiProvider apiRoomsWithCompletion:^(NSArray *rooms, NSError *error) {
        if (!error) {
            ...
            _currentChatRooms = nil;
            _currentChatRooms = [NSMutableDictionary dictionaryWithObjects:rooms forKeys:[rooms valueForKey:@"roomID"]];
...
- (void)connection:(ACConnection *)connection didChangeConnectionStatus:(ACConnectionStatus)connectionStatus {
    ...
    if ([self.delegate respondsToSelector:@selector(chat:didUpdateStatusFromStatus:toStatus:)]) {
...
- (void)connection:(ACConnection *)connection didReceiveMessage:(ACMessageModel *)message {
    ...
    //check if received message is from known room
    if (![_currentChatRooms valueForKey:message.roomID]) {
        
        //create a new room with sender
        [self createRoomWithOpponent:message.senderID completion:^(NSError *error, ACRoomModel *room) {
            if (room && error == nil)
            {
                //put received message as a last message of a new room
                room.lastMessage = message;
                
                //notify the delegate about the new room (and message)
                if ([self.delegate respondsToSelector:@selector(chat:didConnectChatRoom:)]) {
                    [self.delegate chat:self didConnectChatRoom:room];
                }
            }
        }];
    }
...
-(NSArray *)rooms
{
    return self.currentChatRooms.allValues;
}
    ...
    [self performBlockWithValidToken:^(NSError *error) {
        ...
        [self.apiProvider apiRoomsWithCompletion:^(NSArray *rooms, NSError *error) {
            if (!error) {
                ...
                self_weak.userInfoModel.rooms = rooms;
                    ...
                    completion:(void(^)(NSError *error, ACRoomModel *room))completion
{
    ...
    [self performBlockWithValidToken:^(NSError *error) {
        ...
        typeof(self_weak) self = self_weak;
        [self.apiProvider apiCreateRoomWithOpponent:opponentUserId completion:^(NSError *error, NSDictionary *room) {
            ACRoomModel *acRoom = nil;
            if (room && error==nil) {
                acRoom = [[ACRoomModel alloc] initWithDictionary:room];
                _currentChatRooms[acRoom.roomID] = acRoom;
            }
            completion(error, acRoom);
        }];
    }];
}
         ...
         completion:(void(^)(NSError *error, bool isComplete))completion
{
    ...
    [self performBlockWithValidToken:^(NSError *error) {
        ...
        typeof(self_weak) self = self_weak;
        [self.apiProvider apiReadMessage:messageID completion:^(NSError *error, NSDictionary *status) {
            if (status) {
                completion(error, [status[@"status"] intValue] == 200);
            }
            else
                completion(error, NO);
        }];
